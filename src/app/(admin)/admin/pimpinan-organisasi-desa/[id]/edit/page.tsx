...
export default function EditPimpinanOrganisasiDesaPage() {
  ...
  useEffect(() => {
    const fetchPimpinan = async () => {
      try {
        const response = await fetch(`/api/pimpinan-organisasi-desa/${params.id}`);
        if (response.ok) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        throw new Error("Failed to update pimpinan organisasi desa");
      }
    } catch (error) {
      console.error("Error updating pimpinan organisasi desa:", error);
      alert("Gagal mengupdate pimpinan organisasi desa. Silakan coba lagi.");
    } finally {
  ...
  return (
    <div>
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-900">Edit Pimpinan Organisasi Desa</h1>
        <p className="text-gray-600 mt-1">Ubah data pimpinan organisasi desa</p>
      </div>
