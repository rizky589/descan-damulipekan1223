...
export default function EditReportDataPage() {
  ...
  useEffect(() => {
    const fetchReport = async () => {
      try {
        const response = await fetch(`/api/report-data/${params.id}`);
        if (response.ok) {
          const data = await response.json();
          setFormData({
            judul: data.judul || "",
            waktuTerbit: data.waktuTerbit ? new Date(data.waktuTerbit).toISOString().split('T')[0] : "",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert("Gagal mengupdate report data. Silakan coba lagi.");
    } finally {
  ...
  return (
    <div>
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-900">Edit Report Data</h1>
        <p className="text-gray-600 mt-1">Ubah data report</p>
      </div>
